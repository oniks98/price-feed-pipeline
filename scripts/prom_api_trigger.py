...
Статус імпорту:
  ...
  prom_import_status.py і потрапляє в data-latest разом з merged.csv через amend.
  Наступний run читає цей статус в prom_merge_pending.py і при failed об'єднує
  merged_prev.csv з новим merged.csv перед відправкою.
"""
...
from prom_import_status import (
    compute_file_hash,
    load_status,
    record_failure,
    record_success,
...
def main() -> None:
    ...
    # Обчислюємо хеш поточного merged.csv для запису в статус
    merged_path = get_merged_csv_path()
    merged_hash = compute_file_hash(merged_path) if merged_path.exists() else "unknown"
    print(f"🔑 merged.csv hash: {merged_hash}")

    # Завантажуємо поточний статус (для record_failure потрібен попередній стан)
    ...
    # ── Запис статусу ──────────────────────────────────────────
    if last_error:
        new_status = record_failure(current_status, merged_hash)
        save_status(new_status)
        print(f"\n❌ Помилка запуску імпорту ({attempt} спроб):\n{last_error}")
        print(f"   consecutive_failures: {new_status['consecutive_failures']}")
        print(f"   pending_hashes count: {len(new_status['pending_hashes'])}")
        sys.exit(1)

    new_status = record_success(current_status)
    save_status(new_status)
    ...
    print(f"   consecutive_failures скинуто → 0")
