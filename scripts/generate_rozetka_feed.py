...
Генерує фід для Розетки:
  ...
  4. Зберігає результат in data/markets/rozetka_feed.xml

ВАЖЛИВО: Розетка забирає Prom-фід практично в оригінальному вигляді.
  - Теги XML НЕ перейменовуються і НЕ перетворюються.
  - Єдина зміна — множення ціни на коефіцієнт.
  - Русскомовні назви (<name_ru>), описи (<description_ru>) та
    характеристики (<param name="...">) потрапляють у вихідний фід без змін.
...
DEFAULT_COEFFICIENT = Decimal("1.32")
COEF_COLUMN = "coef_rozetka"
...
def apply_prices(
    ...
    """
    Змінює лише <price> — множить на коефіцієнт.
    Усі інші теги (<name_ru>, <description_ru>, <param> тощо)
    залишаються незайманими, бо Розетка їх розуміє нативно.
    """
    converted_count = 0
...
def fill_missing_vendor(xml: str) -> str:
    """Підставляє <vendor> і <country> якщо вони відсутні або порожні.
